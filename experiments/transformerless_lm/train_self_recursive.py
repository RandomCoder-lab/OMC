...
try:
    from addressed_chunks import AddressedChunkRegistry as _AddressedChunkRegistry
    from addressed_param_store import AddressedParamStore as _AddressedParamStore
    from hierarchical_address import assign_address as _assign_address
    _ADDR_SYS_AVAILABLE = True
except ImportError:
    _ADDR_SYS_AVAILABLE = False
    ...
    def boost(self,
        ...
        if self.token_counts is not None:
            ...
            if well_obs.any():
                ...
                # Pad masks and at_bin to full vocab size; chunk tokens get zeros (no boost)
                _bv = boost_vec.shape[0]
                if at_bin.shape[0] < _bv:
                    _pad = _bv - at_bin.shape[0]
                    at_bin = torch.cat([at_bin, at_bin.new_zeros(_pad)])
                    t1 = torch.cat([t1, t1.new_zeros(_pad, dtype=torch.bool)])
                    t2 = torch.cat([t2, t2.new_zeros(_pad, dtype=torch.bool)])
                if t1.any():
...
def autoregressive_generate(model, prompt: torch.Tensor, n_new: int,
    ...
    """
    # Extend vocab list to match vocab_size when chunk tokens have been promoted.
    # Every internal computation using len(vocab) must see the full vocabulary.
    if vocab is not None and vocab_size > len(vocab):
        vocab = list(vocab) + [f'\x00chunk{i}' for i in range(len(vocab), vocab_size)]
    model.eval()
    ...
    with torch.no_grad():
        ...
        for _ in range(n_new):
            ...
            p = substrate_iambic_phase(
                syl_pos, base, vocab_size, newline_mask=None)
            lang_delta += _omniweight_delta(base, p)
            if vocab is not None:
                p = substrate_clause_arc(
                    clause_pos, base, n_chars_local,
                    verb_mask=None, noun_mask=None,
                    adj_mask=None, func_mask=None)
                lang_delta += _omniweight_delta(base, p)
            if open_needs > 0:
                p = substrate_need_fill(
                    open_needs, base, vocab_size, punct_mask=None)
                lang_delta += _omniweight_delta(base, p)
            if vocab is not None:
            ...
            if _fp_tape is not None and _fp_tape.norm() > 1e-8:
                ...
                if (seq.shape[1] >= 1
                    ...
                    _w_agg = 0.0
                    _lead_leaf = -1
                    _leaf_of_tok_raw = fingerprint.sub_clusters[:base.shape[0]].to(base.device)
                    if _leaf_of_tok_raw.shape[0] < base.shape[0]:
                        _leaf_of_tok = torch.cat([
                            _leaf_of_tok_raw,
                            _leaf_of_tok_raw.new_zeros(base.shape[0] - _leaf_of_tok_raw.shape[0])
                        ])
                    else:
                        _leaf_of_tok = _leaf_of_tok_raw
            ...
            if _traj is not None:
                ...
                _sc_raw = fingerprint.sub_clusters[:base.shape[0]].to(base.device)
                if _sc_raw.shape[0] < base.shape[0]:
                    _sc_raw = torch.cat([_sc_raw, _sc_raw.new_zeros(base.shape[0] - _sc_raw.shape[0])])
                _in_leaf = (_sc_raw == _plan_leaf).float()
...
def staged_refine(model, prompt, n_new, vocab_size,
    ...
    with torch.no_grad():
        draft = autoregressive_generate(model, prompt, n_new=n_new,
                                          ...
                                          temperature=temperature, bigram_prior=bigram_prior, vocab=vocab, token_signatures=token_signatures, active_vocab_size=active_vocab_size, class_id_tensor=class_id_tensor, n_classes=n_classes, target_profile=target_profile, fib_pos_table=fib_pos_table)
    stages_out = {}
...
def train_with_self_distillation(name, train_seed, corpus_anchor, val_split,
    ...
    # K is the fixed Fibonacci harmonic basis (89 = F11 positions).
    # K-shrink is retired — chunk depth provides the compression instead.
    sched = lambda s, T: args.K_init
    add_bloom_token = model_bloom_grow   # grows vocab by one token
    _base_vocab_size = vocab_size        # anchor — chunk tokens start above this
    n_params = sum(p.numel() for p in model.parameters())
    _chunk_reg = None
    _param_store = None
    if _ADDR_SYS_AVAILABLE:
        _chunk_reg = _AddressedChunkRegistry()
        print("  [ADDR-SYS] AddressedChunkRegistry initialized", flush=True)
        _param_store = _AddressedParamStore()
        print("  [ADDR-SYS] ... + param_store initialized", flush=True)
    ...
    if _os.path.exists(bloom_checkpoint_path):
        ...
        if _prior_seqs:
            ...
            # If the full corpus text was saved, re-tokenize it and use it as
            # active_base — gives the next run the full grown corpus, not just
            # the 21 diversity-kept bloom sequences.
            _ab_text = _ckpt.get('active_base_text', '')
            if _ab_text and vocab_for_bigram is not None:
                _ab_stoi = {c: i for i, c in enumerate(vocab_for_bigram) if len(c) == 1}
                _ab_toks = [_ab_stoi[c] for c in _ab_text if c in _ab_stoi]
                if len(_ab_toks) > (len(_prior_active_base) if _prior_active_base is not None else 0):
                    _prior_active_base = torch.tensor(_ab_toks, dtype=torch.long)
                    print(f"  [CORPUS RESTORE] {len(_ab_text)} chars → "
                          f"{len(_ab_toks)} base tokens (full corpus inherited)", flush=True)
        ...
        if _ckpt.get('param_store') and _ADDR_SYS_AVAILABLE and _param_store is not None:
            try:
                _ps_prior = _AddressedParamStore.from_dict(_ckpt['param_store'])
                _param_store = _ps_prior
                print(f"  [PARAM-STORE] restored: {_param_store.summary()}", flush=True)
                # Reconstruct embed.weight rows from addressed prototypes.
                # Tokens present in the store get their trained vectors back;
                # any token absent from the store keeps its random init.
                _ew = model.embed.weight.data
                _n_restored = 0
                for _tok_idx, _key in _param_store._token_to_key.items():
                    if _tok_idx < _ew.shape[0]:
                        _proto = _param_store._prototypes.get(_key)
                        if _proto is not None:
                            _ew[_tok_idx] = _proto.to(_ew.device)
                            _n_restored += 1
                print(f"  [PARAM-STORE] restored {_n_restored} embed rows from addressed store",
                      flush=True)
            except Exception as _ps_err:
                print(f"  [PARAM-STORE] restore failed: {_ps_err}", flush=True)
        # Register base tokens into the (possibly restored) param_store so they
        # are tracked for EMA sync at save time. Any token already in the store
        # (from a prior run's save) will have its prototype written back to embed.weight.
        if _param_store is not None and _ADDR_SYS_AVAILABLE:
            _base_ew = model.embed.weight.detach().cpu()
            _n_base_reg = 0
            for _bi in range(min(vocab_size, _base_ew.shape[0])):
                _baddr = _assign_address(_base_ew[_bi])
                _proto = _param_store.register_chunk(_bi, _baddr, _base_ew[_bi])
                if _proto is not None and not torch.equal(_proto, _base_ew[_bi]):
                    model.embed.weight.data[_bi] = _proto.to(model.embed.weight.device)
                    _n_base_reg += 1
            print(f"  [ADDR-SYS] registered {vocab_size} base tokens → "
                  f"{_param_store.n_addresses()} addresses "
                  f"({_n_base_reg} embed rows restored from prior run)", flush=True)

    if _chunk_reg is not None and vocab_for_bigram is not None and _bloom_all_seqs_meta:
        _ws_emb_w = model.embed.weight.detach().cpu()
        _ws_vocab  = vocab_for_bigram
        _n_ws_obs  = 0
        for _ws_seq, *_ in _bloom_all_seqs_meta:
            _ws_list = _ws_seq.tolist()
            _chunk_reg.observe_sequence(_ws_list)
            _ws_text = ''.join(_ws_vocab[t] if t < len(_ws_vocab) else '' for t in _ws_list)
            _ws_tlen = len(_ws_text)
            for _wl in range(8, min(41, _ws_tlen + 1), 4):
                for _wi in range(0, _ws_tlen - _wl + 1, 4):
                    _sub = _ws_text[_wi:_wi + _wl]
                    if len(_sub) >= 8:
                        _chunk_reg.observe_run(_sub, _ws_emb_w, _ws_vocab)
                        _n_ws_obs += 1
        _ws_n_cands = sum(1 for c in _chunk_reg._run_counts.values() if c >= 2)
        print(f"  [CHUNK-WARMUP] {_n_ws_obs} run observations from "
              f"{len(_bloom_all_seqs_meta)} inherited sequences → "
              f"{len(_chunk_reg._run_counts)} unique candidates  "
              f"{_ws_n_cands} promotable (count≥2)", flush=True)
    ...
    # Training batches are drawn from corpus_anchor (50-100%) + active_base to
    # keep the model on a gradient toward the target at all saturation levels.
    ...
    cur_K = None
    _log_every = max(steps_per_cycle // 4, 100)
    eval_every = _log_every  # kept for compatibility
    global_step = 0
    ...
    if corpus_anchor is not None and corpus_anchor.numel() >= 3:
        ...
        _ca_samp_match_floor = max(0.10, 0.40 * (len(_ca_3grams) / 6765) ** (1.0 / _PHI_FOR_SAMPLING))
    ...
    # PRE-TRAINING PHASE: pure CE on corpus_anchor before self-recursive loop.
    # The model must know the corpus before it can creatively explore it.
    _pretrain_steps = getattr(args, 'pretrain_steps', 0)
    if _pretrain_steps > 0 and corpus_anchor is not None:
        _pt_corpus = corpus_anchor.to(_model_device)
        _pt_gen = torch.Generator(); _pt_gen.manual_seed(args.seed + 99)
        print(f"\n[PRE-TRAIN] {_pretrain_steps} steps on full corpus "
              f"({corpus_anchor.numel()} tokens)  target loss < 2.0", flush=True)
        set_K_active_recursive(model, args.K_init)
        for _pt_s in range(_pretrain_steps):
            x, y = sample_tiny_batch(_pt_corpus, args.batch_size, args.seq_len, _pt_gen)
            logits = model(x)
            _pt_loss = substrate_fft_loss(logits, y, vocab_size,
                                          lambda_substrate=args.lambda_sub)
            optimizer.zero_grad(); _pt_loss.backward(); optimizer.step()
            if _pt_s % 1000 == 0 or _pt_s == _pretrain_steps - 1:
                print(f"  [PRE-TRAIN] step {_pt_s:6d}/{_pretrain_steps}  "
                      f"loss={_pt_loss.item():.4f}", flush=True)
        print(f"[PRE-TRAIN] complete — final loss={_pt_loss.item():.4f}\n", flush=True)
    ...
    for cycle in range(n_cycles):
        ...
        _eff_keep = _eff_spc  # anchor-only: no pre-cap, evaluate all generated samples
        ...
        for s in range(steps_per_cycle):
            ...
            if new_K != cur_K:
                ...
                cur_K = new_K
            # Saturation-directed batch sampling: train on corpus_anchor
            # (real Shakespeare) with probability (1 - saturation), so the
            # model gets gradient toward the target when it's furthest away.
            # At full saturation it trains on self-generated corpus.
            _sat_now = (len(_active_run_3gs) / max(len(_ca_3grams), 1)
                        if _ca_3grams else 1.0)
            _anchor_prob = max(0.50, 1.0 - _sat_now)
            if (corpus_anchor is not None
                    and rng.random() < _anchor_prob):
                x, y = sample_tiny_batch(corpus_anchor.to(_model_device),
                                         args.batch_size, args.seq_len, gen)
            else:
            ...
            if global_step % eval_every == 0:
                _n_promoted = (_chunk_reg.n_promoted() if _chunk_reg else 0)
                _mean_chunk_len = (
                    sum(len(n.string) for n in _chunk_reg._nodes.values()) / _n_promoted
                    if _n_promoted else 1.0
                ) if _chunk_reg else 1.0
                _chars_covered = int(cur_K * (1.0 - (_n_promoted / max(1, vocab_size))
                                     + (_n_promoted / max(1, vocab_size)) * _mean_chunk_len))
                print(f"    step {global_step:5d}  loss={loss.item():.4f}"
                      f"  K={cur_K}  chars≈{_chars_covered}"
                      f"  ({time.time()-t0:.1f}s)", flush=True)
            global_step += 1

        # Chunk promotion: scan for L1 candidates and merges at end of each cycle.
        def _haddr_str(addr):
            """Format a Fibonacci harmonic address for display."""
            if addr is None:
                return "?"
            if isinstance(addr, (list, tuple)):
                return ','.join(str(a) for a in addr)
            return str(addr)
        if _chunk_reg is not None and vocab_for_bigram is not None:
            _ew_cpu = model.embed.weight.detach().cpu()
            for _ck_str, _ck_emb, _ck_addr in _chunk_reg.l1_candidates(_ew_cpu, vocab_for_bigram):
                _ck_tier = getattr(model, '_max_bloom_tier_seen', 13)
                _ck_proto = (_param_store.get_prototype(_ck_addr) if _param_store is not None else None)
                _ck_init  = _ck_proto if _ck_proto is not None else _ck_emb
                _ck_idx, bigram_prior = add_bloom_token(
                    model, _ck_init.to(_model_device), vocab_for_bigram, _ck_str, bigram_prior
                )
                vocab_size = model.embed.vocab_size
                _chunk_reg.promote_l1(_ck_str, _ck_idx, _ck_emb, _ck_addr, tier=_ck_tier)
                if _param_store is not None:
                    _param_store.register_chunk(_ck_idx, _ck_addr, _ck_emb)
                if itos_map is not None:
                    itos_map[_ck_idx] = _ck_str
                _ck_sibs = (_param_store.get_siblings(_ck_idx) if _param_store else [])
                print(
                    f"  [CHUNK-L1] tok={_ck_idx}  T{_ck_tier}"
                    f"  @ {_haddr_str(_ck_addr)}"
                    + (f"  siblings={_ck_sibs}" if _ck_sibs else "")
                    + f"  {repr(_ck_str)}",
                    flush=True,
                )
            for _mk_a, _mk_b, _mk_str, _mk_emb, _mk_addr in _chunk_reg.merge_candidates():
                _mk_proto = (_param_store.get_prototype(_mk_addr) if _param_store is not None else None)
                _mk_init  = _mk_proto if _mk_proto is not None else _mk_emb
                _mk_idx, bigram_prior = add_bloom_token(
                    model, _mk_init.to(_model_device), vocab_for_bigram, _mk_str, bigram_prior
                )
                vocab_size = model.embed.vocab_size
                _chunk_reg.promote_merge(_mk_a, _mk_b, _mk_idx, _mk_str, _mk_emb, _mk_addr)
                if _param_store is not None:
                    _param_store.register_chunk(_mk_idx, _mk_addr, _mk_emb)
                if itos_map is not None:
                    itos_map[_mk_idx] = _mk_str
                _mk_node = _chunk_reg._nodes[_mk_idx]
                _mk_sibs = (_param_store.get_siblings(_mk_idx) if _param_store else [])
                print(
                    f"  [CHUNK-MERGE] tok={_mk_idx}  L{_mk_node.level}"
                    f"  T{_mk_node.tier}  {len(_mk_str)}chars"
                    f"  @ {_haddr_str(_mk_addr)}"
                    + (f"  siblings={_mk_sibs}" if _mk_sibs else "")
                    + f"  {repr(_mk_str[:60])}",
                    flush=True,
                )
        ...
        if in_warmup:
            ...
            _wu_chunk_frac = (sum(1 for t in _wu_seq[0].tolist() if t >= _base_vocab_size)
                              / max(1, _wu_seq.shape[1])
                              if _wu_seq is not None and _base_vocab_size else 0.0)
            print(f"  [warmup {cycle+1}] production={_wu_ps:.4f}"
                  f"  chunk_frac={_wu_chunk_frac:.3f}{_cms_str}"
        ...
        if not prod_allows_growth:
            ...
            _cyc_chunk_frac = 0.0
            if cycle_long_seq is not None and _base_vocab_size:
                _cyc_toks = cycle_long_seq[0].tolist()
                _cyc_chunk_frac = sum(1 for t in _cyc_toks if t >= _base_vocab_size) / max(1, len(_cyc_toks))
            _cyc_chunk_str = f"  chunk_frac={_cyc_chunk_frac:.3f}" if _chunk_reg and _chunk_reg.n_promoted() else ""
            print(f"  [cycle {cycle+1}] production={cycle_prod_score:.4f} ≥ gate={_adaptive_gate:.3f}{_cyc_chunk_str} — CORPUS GROWTH OPEN", flush=True)
        ...
        for s_idx in range(_eff_spc):
            ...
            with torch.no_grad():
                draft = autoregressive_generate(
                    ...
                    bigram_prior=bigram_prior, vocab=vocab, token_signatures=token_signatures, active_vocab_size=active_vocab_size, class_id_tensor=class_id_tensor, n_classes=n_classes, target_profile=_target_profile, fib_pos_table=_fib_table, fingerprint=_fingerprint)
            refined_s, _ = staged_refine(
                ...
                bigram_prior=bigram_prior, vocab=vocab, token_signatures=token_signatures, active_vocab_size=active_vocab_size, class_id_tensor=class_id_tensor, n_classes=n_classes, target_profile=_target_profile, fib_pos_table=_fib_table)
        ...
        kept = samples  # anchor-only gate: all samples evaluated, no pre-cap
        ...
        for ref_seq, cr in kept:
            ...
            if _ca_3grams is not None:
                ...
                _samp_match = (sum(1 for g in _s_3gs if g in _ca_3grams)
                               / max(1, len(_s_3gs)))
            # Cold-start quality floor: linearly interpolates from 1/phi^4 (0.146)
            # when corpus is tiny to 1/phi^2 (0.382) once corpus reaches 5x seed size.
            # Prevents quality deadlock when the model hasn't yet bootstrapped.
            _cs_factor = min(1.0, active_base.numel() / max(1, orig_seed_chars * 20))
            _q_floor = (1.0/(_PHI_FOR_SAMPLING**2) * _cs_factor
                        + 1.0/(_PHI_FOR_SAMPLING**4) * (1.0 - _cs_factor))
            passes_q = True   # quality gate removed — anchor is the corpus-validity check
            ...
            if _ca_3grams is not None:
                ...
                _novel_ow_score = len(_ref_novel_3gs) * _run_ow * (1.0 - (1.0/_PHI_FOR_SAMPLING) * _saturation)
                _novel_threshold = (1.0/(_PHI_FOR_SAMPLING**2)) * (1.0 + _saturation)
            passes_novel = True   # omniw novelty gate removed — anchor handles corpus validity
    ...
    _bloom_save = {
        ...
        'chunk_registry': (_chunk_reg.to_dict() if _chunk_reg else None),
        'param_store':    (_param_store.to_dict() if _param_store else None),
    }
    # Decode active_base to text so the next run can inherit the full grown corpus.
    # Chunk tokens are expanded to their constituent characters via chunk_registry.
    if vocab_for_bigram is not None:
        _ab_itos = {i: vocab_for_bigram[i] for i in range(len(vocab_for_bigram))}
        if _chunk_reg is not None:
            _ab_text_out = _chunk_reg.decode_sequence(active_base.tolist(), _ab_itos)
        else:
            _ab_text_out = ''.join(_ab_itos.get(i, '') for i in active_base.tolist()
                                   if len(_ab_itos.get(i, '')) == 1)
        _bloom_save['active_base_text'] = _ab_text_out
        print(f"  [CORPUS SAVE] {len(_ab_text_out)} chars of corpus → checkpoint", flush=True)
    # EMA sync all registered tokens back into param_store before saving,
    # so the next run inherits fully-trained embedding vectors.
    if _param_store is not None:
        _ew_final = model.embed.weight.detach().cpu()
        for _tok_idx in list(_param_store._token_to_key.keys()):
            if _tok_idx < _ew_final.shape[0]:
                _param_store.update_prototype(_tok_idx, _ew_final[_tok_idx], alpha=1.0)
        # Re-serialize with updated prototypes
        _bloom_save['param_store'] = _param_store.to_dict()
    ...
    # Save model state separately so generate_interpolated.py can load it.
    _model_ckpt_path = Path(bloom_checkpoint_path).parent / "bloom_checkpoint_model.pt"
    torch.save({
        'model_state': model.state_dict(),
        'vocab':       list(itos.values()) if hasattr(itos, 'values') else vocab,
        'seq_len':     args.seq_len,
        'd_model':     args.d_model,
        'best_creativity': best_creativity,
        'best_production': best_production,
        'active_base_size': len(active_base),
    }, str(_model_ckpt_path))
    print(f"  [MODEL SAVED] → {_model_ckpt_path}", flush=True)
...
def main():
    ...
    parser.add_argument("--pretrain-steps", type=int, default=10000,
                          help="Steps of pure CE on full corpus before self-recursive "
                               "loop. 10000 ≈ 10 passes over Shakespeare; enough for "
                               "the model to know the corpus before creative generation.")
    ...
    # corpus_anchor = the full training corpus (up to val split) so anchor 3-grams
    # cover all of Shakespeare and the model trains on the real target, not just
    # a 512-token excerpt. This is the passage the model must learn to produce.
    corpus_anchor = encoded[:val_start].clone()
