...
def substrate_fft_loss(logits: torch.Tensor, targets: torch.Tensor,
    ...
    # Fibonacci-tier-weighted mean over band dimension: F(k)/phi^(pi*k) decay.
    # Low-frequency structural bands (k=0, freq=1) get full gradient; high-
    # frequency noise bands (k=6, freq=21) get near-zero gradient.
    _fib_freqs_list = [1, 2, 3, 5, 8, 13, 21]
    _band_w = torch.tensor(
        [f / (PHI ** (math.pi * k)) for k, f in enumerate(_fib_freqs_list)],
        dtype=torch.float32, device=logits.device,
    )
    _band_w = _band_w / _band_w.sum()
    _band_err = ((pred_cos - tgt_cos) ** 2 + (pred_sin - tgt_sin) ** 2).mean(dim=(0, 1))  # [K]
    fft_mismatch = (_band_err * _band_w).sum()
...
def substrate_omniweight_loss(logits: torch.Tensor, targets: torch.Tensor,
    ...
    # Anti-stagnation contribution to the ledger — continuous OmniWeight kernel.
    # Replaces the 3-step staircase (flat delta between thresholds, zero gradient)
    # with a phi-decaying contribution per Fibonacci threshold.  For each tier T
    # that count >= T, the contribution decays as phi^(-excess/T), giving smooth
    # gradient everywhere above count=8.  Tier weights (1, 1, 2) match the old
    # (-1, -2, -4) * LOG_PHI_PI staircase at exact threshold crossings.
    delta = torch.zeros_like(counts)
    for _thresh, _w in [(8, 1.0), (13, 1.0), (21, 2.0)]:
        mask = counts >= float(_thresh)
        _e = (counts - _thresh).clamp(min=0.0) / max(_thresh, 1)
        delta = delta - mask.to(dtype) * (_w * _LOG_PHI_PI * (_PHI ** (-_e)))
