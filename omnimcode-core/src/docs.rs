...
print(files)  # ["best_of_n_improve.omc", "multi_agent_debate.omc", ...]"#,
    ...
    BuiltinDoc { name: "haddr", category: "substrate", signature: "(text) -> dict", description: "Equal-area dodecahedral content-address {face, sub_face, zeck} of text. Uniform φ-address (χ²≈17), no dictionary.", example: "haddr(\"fibonacci\")  // {face:.., sub_face:.., zeck:[..]}", unique_to_omc: true },
    BuiltinDoc { name: "haddr_distance", category: "substrate", signature: "(a, b) -> float", description: "Coarse address distance; a/b are haddr dicts or strings. Face 3.0, sub 1.0, 0.5 per zeck symdiff.", example: "haddr_distance(haddr(\"a\"), haddr(\"b\"))", unique_to_omc: true },
    BuiltinDoc { name: "haddr_face", category: "substrate", signature: "(text) -> int", description: "Dodecahedral face (0-11) of text's uniform φ-address.", example: "haddr_face(\"gcd\")  // 0..11", unique_to_omc: true },
    ...
    // ──────────────────────────────────────────────────────────────────────────────
    // Substrate-into-core (v1.8.x): content-addressing, addressable heap, similarity,
    // self-modification, synthesis, dual-band. See SUBSTRATE.md.
    // ──────────────────────────────────────────────────────────────────────────────
    BuiltinDoc { name: "crt_pe", category: "substrate", signature: "(pos: int, [moduli: int[]]) -> float[]",
        description: "CRT positional encoding: normalized residues [(pos % m)/m]; default moduli {5,8,13,21} are pairwise-coprime → unique over lcm 10920. Attenuable positional feature.",
        example: "crt_pe(7)  // [0.4, 0.875, 0.538, 0.333]", unique_to_omc: true },
    BuiltinDoc { name: "value_addr", category: "substrate", signature: "(value) -> dict",
        description: "Dodecahedral content-address {face, sub_face, zeck} of any value (structural — same content, same address).",
        example: "value_addr([1,2,3])", unique_to_omc: true },
    BuiltinDoc { name: "value_hash", category: "substrate", signature: "(value) -> string",
        description: "FNV1a-64 content hash (decimal string) of any value — the content-addressed-heap key.",
        example: "value_hash([1,2,3])", unique_to_omc: true },
    BuiltinDoc { name: "same_value", category: "substrate", signature: "(a, b) -> bool",
        description: "O(1) semantic equality by content hash — structurally equal values collide regardless of provenance.",
        example: "same_value([1,2,3], [1,2,3])  // true", unique_to_omc: true },
    BuiltinDoc { name: "cas_put", category: "substrate", signature: "(value) -> string",
        description: "Store a value in the content-addressed heap (persists to ~/.omc/cas across runs); returns its content key. Identical content dedups.",
        example: "h k = cas_put([10,20,30]);", unique_to_omc: true },
    BuiltinDoc { name: "cas_get", category: "substrate", signature: "(key: string) -> value|null",
        description: "Retrieve a value from the content-addressed heap by key (memory, then disk).",
        example: "cas_get(k)  // [10,20,30]", unique_to_omc: true },
    BuiltinDoc { name: "cas_has", category: "substrate", signature: "(key: string) -> bool",
        description: "Is a value stored at this content key (memory or disk)?",
        example: "cas_has(k)  // true", unique_to_omc: true },
    BuiltinDoc { name: "locality_fp", category: "substrate", signature: "(text, [bigram: int]) -> float[]",
        description: "Locality-bearing fingerprint (normalized byte histogram; bigram=1 → 4096-dim). Similar content → similar vector — the similarity primitive (unlike haddr).",
        example: "locality_fp(\"quicksort\")", unique_to_omc: true },
    BuiltinDoc { name: "locality_sim", category: "substrate", signature: "(a, b, [bigram: int]) -> float",
        description: "Cosine similarity of two strings' locality fingerprints, in [0,1].",
        example: "locality_sim(\"quicksort\",\"quick_sort\")  // ~0.95", unique_to_omc: true },
    BuiltinDoc { name: "locality_nearest", category: "substrate", signature: "(query, candidates: array, [bigram: int]) -> int",
        description: "Index of the candidate most similar to query by locality cosine (the retrieval primitive — use this, not haddr, for similarity).",
        example: "locality_nearest(\"quicksrt\", [\"gcd\",\"quicksort\"])  // 1", unique_to_omc: true },
    BuiltinDoc { name: "nearest_fn", category: "substrate", signature: "(need: string) -> string",
        description: "Name of the defined function closest to `need` by content locality (typo/variant-tolerant; not semantic NL→code).",
        example: "nearest_fn(\"quicksrt\")  // \"quicksort\"", unique_to_omc: true },
    BuiltinDoc { name: "call_nearest", category: "substrate", signature: "(need: string, args) -> value",
        description: "Dispatch to the nearest-named function (by locality) and call it with args (array or single value).",
        example: "call_nearest(\"gdc\", [48,36])  // gcd(48,36)=12", unique_to_omc: true },
    BuiltinDoc { name: "fn_swap_verified", category: "substrate", signature: "(name: string, new_source: string, test_source: string) -> dict",
        description: "Safe self-modification: install candidate code for `name`, run the test in a sandbox, keep only if it passes (returns truthy) — else roll back. Returns {accepted, error, result}.",
        example: "fn_swap_verified(\"f\", src, \"f(2)==4\")", unique_to_omc: true },
    BuiltinDoc { name: "fns_on_face", category: "substrate", signature: "(face: int) -> string[]",
        description: "Defined functions whose name-address lands on dodecahedral `face` (0-11) — exact-key function bucketing.",
        example: "fns_on_face(3)", unique_to_omc: true },
    BuiltinDoc { name: "gen_omc", category: "substrate", signature: "([seed: int]) -> string",
        description: "Generate a valid-by-construction OMC program (always parses + runs). Pair with code_parse_check/eval_omc/fn_swap_verified for generate→verify→accept.",
        example: "h prog = gen_omc(7);", unique_to_omc: true },
    BuiltinDoc { name: "gen_at", category: "substrate", signature: "(address_or_text) -> string",
        description: "Address-conditioned synthesis: the same address/need deterministically maps to the same valid-by-construction program.",
        example: "gen_at(\"sort a list\")", unique_to_omc: true },
    BuiltinDoc { name: "hbit_harmony", category: "duals", signature: "(alpha: int, beta: int) -> int",
        description: "Two-band resonance 0..1000 (1000 = in tune). 1000·harmony(α,β) over the Fibonacci attractor lattice.",
        example: "hbit_harmony(8, 8)  // 1000", unique_to_omc: true },
    BuiltinDoc { name: "hbit_divergence", category: "duals", signature: "(alpha: int, beta: int) -> int",
        description: "Two-band divergence 0..1000 (0 = in tune) = the gate value. Trust a β skip while low; fall back to α when high.",
        example: "hbit_divergence(8, 977)  // 947", unique_to_omc: true },
    BuiltinDoc { name: "band_divergence", category: "duals", signature: "(fn_name: string) -> int",
        description: "α/β divergence (0..1000) recorded on the last call to a @dualband function, or -1 if none.",
        example: "band_divergence(\"sq\")", unique_to_omc: true },
    BuiltinDoc { name: "band_route", category: "duals", signature: "(fn_name: string) -> string",
        description: "The gate's routing recommendation from the last @dualband divergence: fast-substrate / cached-exact / linear / unmeasured.",
        example: "band_route(\"sq\")", unique_to_omc: true },
    BuiltinDoc { name: "phi_shadow", category: "duals", signature: "(value) -> int",
        description: "Attach the harmonic SHADOW band β = nearest Fibonacci attractor of α. The value carries both bands; β rides through arithmetic; harmony()/value_divergence() read the drift.",
        example: "bands(phi_shadow(10))  // [10, 8]", unique_to_omc: true },
    BuiltinDoc { name: "bands", category: "duals", signature: "(value) -> int[]",
        description: "Inspect a value's two bands as [α, β] (β = α when single-band).",
        example: "bands(phi_shadow(10) + 3)  // [13, 11]", unique_to_omc: true },
    BuiltinDoc { name: "value_divergence", category: "duals", signature: "(value) -> int",
        description: "Per-value harmonic drift = 1000·(1-harmony(α,β)); 0 = on the lattice, high = dissonant (α is the truth).",
        example: "value_divergence((phi_shadow(50)+1)*3)  // 933", unique_to_omc: true },
];
