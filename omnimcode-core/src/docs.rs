...
print(files)  # ["best_of_n_improve.omc", "multi_agent_debate.omc", ...]"#,
    ...
    BuiltinDoc { name: "amem_new", category: "substrate", signature: "() -> handle",
        description: "Allocate an in-core addressed-memory datastore (write-don't-train kNN + IVF). Hold the handle; pass it to amem_write/index/search/len.",
        example: "h m = amem_new();", unique_to_omc: true },
    BuiltinDoc { name: "amem_write", category: "substrate", signature: "(handle, key_array, value) -> count",
        description: "WRITE a (key_vector -> value) entry with no training (O(1)); returns the entry count. A write invalidates any prior IVF index.",
        example: "amem_write(m, [0.1, 0.9], 42);", unique_to_omc: true },
    BuiltinDoc { name: "amem_index", category: "substrate", signature: "(handle, [nlist=64], [iters=8], [seed=0]) -> cells",
        description: "Build the IVF (k-means Voronoi) index over the written keys; returns the cell count. ~99-100% of brute-force recall at far fewer comparisons.",
        example: "amem_index(m, 256);", unique_to_omc: true },
    BuiltinDoc { name: "amem_search", category: "substrate", signature: "(handle, query_array, [k=5], [nprobe=4]) -> array",
        description: "Return the values of the k nearest written keys to the query vector (probes nprobe IVF cells, brute-force if unindexed). Addressing decides WHERE to look.",
        example: "amem_search(m, [0.1, 0.9], 3);  // [42, ...]", unique_to_omc: true },
    BuiltinDoc { name: "amem_len", category: "substrate", signature: "(handle) -> int",
        description: "Number of entries written to the datastore behind this handle.",
        example: "amem_len(m);  // 1", unique_to_omc: true },
